      ...
      try {
        ...
        const dbProduct = await this.prisma.produto.create({
          data: {
            ...
            nicho: Array.isArray(analysisJson?.nicho) ? analysisJson.nicho.join(', ') : analysisJson?.nicho || null,
            publicoAlvo: Array.isArray(analysisJson?.publico_alvo) ? analysisJson.publico_alvo.join(', ') : analysisJson?.publico_alvo || null,
            intencaoCompra: Array.isArray(analysisJson?.intencao_compra) ? analysisJson.intencao_compra.join(', ') : analysisJson?.intencao_compra || null,
  ...
  async publishManualOffer(analysisData: any) {
    ...
    const dbProduct = await this.prisma.produto.create({
      data: {
        ...
        linkOriginal: link,
        dataPromocao: new Date(),
        nicho: Array.isArray(analysisData?.nicho) ? analysisData.nicho.join(', ') : analysisData?.nicho || null,
        publicoAlvo: Array.isArray(analysisData?.publico_alvo) ? analysisData.publico_alvo.join(', ') : analysisData?.publico_alvo || null,
        intencaoCompra: Array.isArray(analysisData?.intencao_compra) ? analysisData.intencao_compra.join(', ') : analysisData?.intencao_compra || null,
